...
(async () => {
	...
	for ( let aMAIL of ACCOUNTs ) {
	    ...
	    {
		const TEMP =
		      await openBRuntilSuccess('https://www.kanto.co.jp/corp/officejp.html', 10000);
		localBrowser = TEMP.BR; localPage = TEMP.PG;
	    ...
	    {
		var TBL1 = await TBL[2].$$('tr');
		var BRA = await (await (await TBL1[0].$('th > strong')).getProperty('innerHTML')).jsonValue();
		BRA = BRA.replace(/[ \t\n　]+/gsm,'').replace(/<.+?>/gsm,'');

		for (let i = 1; i < TBL1.length; i++) {
		    //var ENT = await (await TBL1[i].getProperty('innerHTML')).jsonValue(); console.log(ENT);
		    var NAME = await (await (await TBL1[i].$('th')).getProperty('textContent')).jsonValue();
		    NAME = NAME.replace(/[ \t\n　]+/gsm,'').replace(/　/,'');
		    var POST = await (await (await TBL1[i].$('td')).getProperty('innerHTML')).jsonValue();
		    POST = POST.replace(/.+?〒/sm,'').replace(/　.*/sm,'').replace(/^./,''); //console.log('POST:'+POST);
		    var ADR = await (await (await TBL1[i].$('td')).getProperty('innerHTML')).jsonValue();
		    ADR = ADR.replace(/.+?<br>/sm,'').replace(/<br>.*/sm,'').replace(/[ \t\n]+/gsm,'');
		    var TEL  = await (await (await TBL1[i].$('td')).getProperty('innerHTML')).jsonValue();
		    TEL = TEL.replace(/.+?<br>/sm,'').replace(/.+?<br>/sm,'').replace(/.FAX.*/sm,'').replace(/[ \t\n]+/gsm,'').replace(/TEL\./sm,'');
		    var FAX  = await (await (await TBL1[i].$('td')).getProperty('textContent')).jsonValue();
		    FAX = FAX.replace(/[ \t\n]+/gsm,'').replace(/.+?FAX\.([0-9\-]+).*/sm,'$1');
		    console.log(BRA+','+NAME+','+ADR+','+POST+','+TEL+','+FAX);
		}
